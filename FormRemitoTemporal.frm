...
Private Sub ImprimirRemito()
    ...
    'Modificaciones remitos nuevos 2025-06-06
    x = 0
    Y = 0
    'x = 0
    'y = 0
                    ...
                    While ValorCelda <> ""
                        'Imprimo el detalle
                           ...
                           ' Printer.Print RemD!cantidad
                            Printer.Print Format(FG1.text, "#,###,###,#0.00")
                            FG1.Col = 1
...
Private Sub BotonGrabar_Click()
        ...
        'else
            ...
            rstRemitoC.Fields!IdSucursal = CLng(Left(cmbSucursales.text, 1))
...
Private Sub Cargo()
    ...
    Set db = DBEngine.OpenDatabase(ruta)
    Set rstEmpleado = db.OpenRecordset("Empleados", dbOpenDynaset)
    Set rstIva = db.OpenRecordset("CondicionIVA", dbOpenDynaset)
    Set rstSucursales = db.OpenRecordset("Sucursales", dbOpenDynaset)
    ...
    Do While Not rstSucursales.EOF
        cmbSucursales.AddItem rstSucursales!IdSucursal & " - " & rstSucursales!NombreSucursal
        rstSucursales.MoveNext
    Loop
    
    cmbSucursales.ListIndex = 1
    
    rstIva.MoveFirst
    Do While Not rstIva.EOF
        cmbIVA.AddItem rstIva!IdCondicionIVA & " - " & rstIva!Descripcion
        rstIva.MoveNext
    Loop
