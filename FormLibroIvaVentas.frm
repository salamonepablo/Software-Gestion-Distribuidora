...
Private Sub GenerarArchivoAlicuotas()
  ...
  'Genero el archivo para SIAP de Alicuotas
    ...
    If Not rstSIAP.EOF Then
        rstSIAP.MoveFirst
     Else
        MsgBox ("No Hay Facturas en el archivo")
        Exit Sub
    End If
 ...
 'Ahora las NOTA DE CREDITO
    ...
    If Not rstSIAP.EOF Then
        rstSIAP.MoveFirst
     Else
        MsgBox ("No Hay Notas de Crédito en el archivo")
...
'    Close #1
   ...
   'Ahora las NOTA DE DEBITO
    
    vSQL = "SELECT * FROM NotaDebitoC WHERE FechaDebito >=" & Desde & " AND FechaDebito <=" & Hasta & " Order By TipoDebito, FechaDebito, NroDebito"
    ...
    If Not rstSIAP.EOF Then
        rstSIAP.MoveFirst
     Else
        MsgBox ("No Hay Notas de Débito en el archivo")
    ...
    While Not rstSIAP.EOF
        'Punto 01 Tipo de Comprobante
             If rstSIAP!TipoDebito = "A" Then linea = linea & "002"
             If rstSIAP!TipoDebito = "B" Then linea = linea & "008"
        
        'Punto 02 Punto de venta
             linea = linea & "00004"
             
        'Punto 03 Numero de comprobante
             NroFac = rstSIAP!NroDebito
        ...
        'Punto 04 Importe Neto Gravado
             Tt = Format(CStr(rstSIAP!SubTotalDebito), "#,###,###,#0.00")
             TotalFac = Split(Tt, ",", -1)
             TtFac = TotalFac(0)
             For I = 1 To (13 - Len(TotalFac(0)))
                 TtFac = "0" & TtFac
...
Private Function DatosClientes(Cliente As Long) As String
    ...
    If Not tClientes.NoMatch Then
        ...
        If Len(tClientes!CUIT) = 11 Then
            If Cliente = 999 Then DatosClientes = 96
            DatosClientes = 80
          Else
            DatosClientes = 96
        End If
...
Private Sub titulos()
    ...
    MSFlexGrid2.ColWidth(12) = 1700
    MSFlexGrid2.ColAlignment(12) = flexAlignCenterCenter
   
    MSFlexGrid2.Col = 13
    MSFlexGrid2.text = "Provincia"
    MSFlexGrid2.ColWidth(13) = 1800
    MSFlexGrid2.ColAlignment(13) = flexAlignCenterCenter
...
'Ahora las Nota de Crédito
    ...
    If Not rstSIAP.EOF Then
        rstSIAP.MoveFirst
     Else
        MsgBox ("No Hay Notas de Credito en el archivo")
...
'Ahora las Nota de Débito
    ...
    While Not rstSIAP.EOF
        'Punto 01 Fecha de comprobante
            linea = Format(rstSIAP!FechaDebito, "yyyymmdd")
        ...
        'Punto 09 Importe total operacion
            Tt = Format(CStr(rstSIAP!TotalDebito), "#,###,###,#0.00")
            TotalFac = Split(Tt, ",", -1)
            TtFac = TotalFac(0)
            For I = 1 To (13 - Len(TotalFac(0)))
                TtFac = "0" & TtFac
        ...
        'Punto 12 Importe de operaciones exentas
            Tt = Format(CStr(BuscaImporteExento(rstSIAP!NroDebito, rstSIAP!TipoDebito)), "#,###,###,#0.00")
            TotalEx = Split(Tt, ",", -1)
            TtEx = TotalEx(0)
            For I = 1 To (13 - Len(TotalEx(0)))
                TtEx = "0" & TtEx
            Next I
        ...
        'Punto 21
            linea = linea & "000000000000000"
            linea = linea & Format(rstSIAP!FechaDebito, "yyyymmdd")
...
Private Function BuscaImporteExento(NroDebito, TipoDebito) As Double

  On Error GoTo Captcha
  
  vSQL = "SELECT * FROM NotaDebitoD WHERE NroDebito =" & NroDebito & " AND TipoDebito ='" & TipoDebito & "' Order By ItemDebito"
'  MsgBox (vSQL)
    
  Set tDebitoD = db.OpenRecordset(vSQL, dbOpenDynaset)
  
  tDebitoD.MoveFirst
  BuscaImporteExento = 0
  
  While Not tDebitoD.EOF
    If tDebitoD!IdCodProd = "CHE" Then
        BuscaImporteExento = BuscaImporteExento + tDebitoD!totalLinea
    End If
    tDebitoD.MoveNext
  Wend
...
On Error GoTo Error_Handler
    ...
    eseqeleND = "SELECT * FROM NotaDebitoC WHERE FechaDebito >=" & Desde & " AND FechaDebito <=" & Hasta & " Order By TipoDebito, FechaDebito, NroDebito"
    
    'MsgBox (eseqele)
    
    Set rst = db.OpenRecordset(eseqele, dbOpenDynaset)
    Set rstNC = db.OpenRecordset(eseqeleNC, dbOpenDynaset)
    Set rstND = db.OpenRecordset(eseqeleND, dbOpenDynaset)
   ...
   While Not rst.EOF
         ...
         MSFlexGrid2.Col = 7
         MSFlexGrid2.text = Format(rst.Fields!SubTotalFactura, "#,###,###,#0.00")
         neto = rst.Fields!SubTotalFactura
         
         MSFlexGrid2.Col = 8
         MSFlexGrid2.text = Format(0, "#,###,###,#0.00")
         
         MSFlexGrid2.Col = 9
         MSFlexGrid2.text = Format(rst.Fields!ImportePercepIIBB, "#,###,###,#0.00")
         percepcion = rst.Fields!ImportePercepIIBB
         MSFlexGrid2.Col = 10
         'iva = (neto * 21) / 100
         iva = Format(rst.Fields!totalIva, "#,###,###,#0.00")
         MSFlexGrid2.text = Format(iva, "#,###,###,#0.00")
         'total = neto + percepcion + iva
         total = Format(rst.Fields!TotalFactura, "#,###,###,#0.00")
         MSFlexGrid2.Col = 11
         MSFlexGrid2.text = Format(total, "#,###,###,#0.00")
   ...
   While Not rstNC.EOF
         ...
         MSFlexGrid2.Col = 7
         MSFlexGrid2.text = (Format(rstNC.Fields!SubTotalNotaCredito, "#,###,###,#0.00")) * -1
         ...
         MSFlexGrid2.Col = 9
         MSFlexGrid2.text = (Format(rstNC.Fields!ImportePercepIIBB, "#,###,###,#0.00")) * -1
         percepcion = rstNC.Fields!ImportePercepIIBB
         MSFlexGrid2.Col = 10
         'iva = (neto * 21) / 100
         iva = (Format(rstNC.Fields!totalIva, "#,###,###,#0.00")) * -1
         MSFlexGrid2.text = (Format(iva, "#,###,###,#0.00"))
         'total = neto + percepcion + iva
         total = (Format(rstNC.Fields!TotalNotaCredito, "#,###,###,#0.00")) * -1
         MSFlexGrid2.Col = 11
         MSFlexGrid2.text = (Format(total, "#,###,###,#0.00"))
  ...
  If Not rstND.EOF Then rstND.MoveFirst

   While Not rstND.EOF
         
    If Not Exento(rstND.Fields!NroDebito, rstND.Fields!TipoDebito) Then
         ...
         MSFlexGrid2.text = "ND"
         MSFlexGrid2.Col = 2
         MSFlexGrid2.text = rstND.Fields!NroDebito
         MSFlexGrid2.Col = 0
         MSFlexGrid2.text = "0004"
         
         MSFlexGrid2.Col = 3
         MSFlexGrid2.text = rstND.Fields!TipoDebito
         MSFlexGrid2.Col = 4
         MSFlexGrid2.text = Format(rstND.Fields!FechaDebito, "dd-MMM-yyyy")
         
         '**** Busco datos Cliente
         
       
         CodigoClie = rstND.Fields!CodCliente
         ...
         MSFlexGrid2.Col = 7
         MSFlexGrid2.text = Format(rstND.Fields!SubTotalDebito, "#,###,###,#0.00")
         neto = rstND.Fields!SubTotalDebito
         
         MSFlexGrid2.Col = 8
         MSFlexGrid2.text = Format(0, "#,###,###,#0.00")
         
         MSFlexGrid2.Col = 9
         MSFlexGrid2.text = Format(rstND.Fields!ImportePercepIIBB, "#,###,###,#0.00")
         percepcion = rstND.Fields!ImportePercepIIBB
         MSFlexGrid2.Col = 10
         'iva = (neto * 21) / 100
         iva = Format(rstND.Fields!totalIva, "#,###,###,#0.00")
         MSFlexGrid2.text = Format(iva, "#,###,###,#0.00")
         'total = neto + percepcion + iva
         total = Format(rstND.Fields!TotalDebito, "#,###,###,#0.00")
         MSFlexGrid2.Col = 11
         MSFlexGrid2.text = Format(total, "#,###,###,#0.00")
    ...
    Else
         ...
         MSFlexGrid2.Col = 8
         MSFlexGrid2.text = Format(rstND.Fields!TotalDebito, "#,###,###,#0.00")
...
Private Function Exento(NroDebito, TipoDebito) As Boolean

    Exento = False
    
    sqlNDD = "SELECT * FROM NotaDebitoD WHERE NroDebito =" & NroDebito & " AND TipoDebito='" & TipoDebito & "' Order By ItemDebito"
    
    'MsgBox (sqlNDD)
    
    Set rstND = db.OpenRecordset(sqlNDD, dbOpenDynaset)
    
    rstND.MoveFirst
    While Not rstND.EOF
              
        If rstND!IdCodProd = "CHE" Then Exento = True
