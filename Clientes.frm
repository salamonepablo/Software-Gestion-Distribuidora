...
Private Sub ChequearCUIT()
    
    'Set BaseSPC2 = OpenDatabase(App.Path & "\DB_SPC_SI.mdb")
        
        'Tabla Clientes
    Set tC = BaseSPC.OpenRecordset("Clientes", dbOpenTable)
    
    tC.Index = "IndxCUIT"
    tC.Seek "=", txtCUIT.text
    
    If Not tC.NoMatch Then
        MsgBox ("Cliente Existente, Registrado con Nro: " & tC!IdCliente)
        txtCUIT.SetFocus
    End If
...
Private Sub txtCUIT_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = vbKeyF4 Then
        If BuscarPorCUIT(txtCUIT.text) Then
            Call Mostrar
        Else
            MsgBox ("CUIT no Encontrado !!!")
            Resume Next
        End If
    End If
...
End Sub
Private Function BuscarPorCUIT(CUIT) As Boolean
    tClientes.Index = "IndxCUIT"
...
Dim Teclas As String
    ...
    Teclas = "1234567890.," & Chr(vbKeyBack) & Chr(vbKeyF4)
